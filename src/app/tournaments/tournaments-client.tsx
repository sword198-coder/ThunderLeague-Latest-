...
export function TournamentsClient({
  ...
  const [approvedCounts, setApprovedCounts] = useState<Map<string, number>>(initialCounts);
  ...
  useEffect(() => {
    ...
    supabase.from("tournament_participants")
      ...
      .then(({ data }) => {
        if (data && !cancelled) {
          setMyParticipation(new Map(data.map((m) => [m.tournament_id, m.status])));
        }
      });
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "tournaments" }, (payload) => {
        ...
        if (payload.eventType === "INSERT") {
          ...
          setTournaments((prev) => prev.map((t) => t.id === payload.new.id ? { ...t, ...payload.new as Tournament } : t));
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "tournament_participants" }, (payload) => {
        ...
        if (payload.eventType === "INSERT" || payload.eventType === "UPDATE") {
          const p = payload.new as { tournament_id: string; status: string };
          if (p.status === "approved") {
            setApprovedCounts((prev) => new Map(prev).set(p.tournament_id, (prev.get(p.tournament_id) ?? 0) + 1));
          } else {
            setApprovedCounts((prev) => {
              const next = new Map(prev);
              const current = next.get(p.tournament_id) ?? 0;
              if (current > 0) next.set(p.tournament_id, current - 1);
              return next;
            });
          }
        } else if (payload.eventType === "DELETE") {
          const p = payload.old as { tournament_id: string; status: string };
          if (p.status === "approved") {
            setApprovedCounts((prev) => {
              const next = new Map(prev);
              const current = next.get(p.tournament_id) ?? 0;
        ...
        if (user) {
          const pNew = payload.new as { user_id?: string; tournament_id: string; status: string } | null;
          const pOld = payload.old as { user_id?: string; tournament_id: string; status: string } | null;
          const matchNew = pNew?.user_id === user.id;
          const matchOld = pOld?.user_id === user.id;
          if (matchNew) {
            setMyParticipation((prev) => new Map(prev).set(pNew!.tournament_id, pNew!.status));
          } else if (matchOld) {
            setMyParticipation((prev) => {
              const next = new Map(prev);
              next.set(pOld!.tournament_id, pOld!.status);
              return next;
            });
          }
        }
  ...
  const active = useMemo(() => tournaments.filter((t) => getEffectiveStatus(t) === "active"), [tournaments, getEffectiveStatus]);
  const upcoming = useMemo(() => tournaments.filter((t) => getEffectiveStatus(t) === "upcoming"), [tournaments, getEffectiveStatus]);
  const past = useMemo(() => tournaments.filter((t) => { const s = getEffectiveStatus(t); return s === "completed" || s === "cancelled"; }), [tournaments, getEffectiveStatus]);
